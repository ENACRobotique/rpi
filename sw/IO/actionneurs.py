...
VALEURS_ACTIONNEURS = {PosTentacle.BAS.value: 500, PosTentacle.DROP.value: 550, PosTentacle.RETOURNE.value: 1000, PosTentacle.HAUT.value: 820, PosTentacle.THERMO.value: 630}
...
class IO_Manager:
    ...
    def moveG(self, position : PosTentacle,timeout=2):
        ...
        while((time.time()-debut < timeout)):
            if self.ready(Actionneur.bicepsG.value,VALEURS_ACTIONNEURS[position.value],50) and self.ready(Actionneur.tricepsG.value,VALEURS_ACTIONNEURS[position.value],50):
                return True
            time.sleep(0.1)
        return False
    ...
    def moveD(self, position : PosTentacle,timeout=2):
        ...
        while (time.time()-debut < timeout) :
            if self.ready(Actionneur.bicepsD.value,VALEURS_ACTIONNEURS[position.value],50) and self.ready(Actionneur.tricepsD.value,VALEURS_ACTIONNEURS[position.value],50) :
                return True
            time.sleep(0.1)
        return False
