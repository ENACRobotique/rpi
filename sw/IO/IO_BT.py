...
class MoveBrasD(py_trees.behaviour.Behaviour):
    ...
    def initialise(self) -> None:
        print(f"Move le bras Droit {self.pos}")
        
    def update(self):
        if self.robot.actionneurs.moveD(self.pos):
...
class MoveBrasG(py_trees.behaviour.Behaviour):
    def __init__(self, pos:PosTentacle):
        ...
        if pos == PosTentacle.DROP:
            self.pos_info = "DROP"
        super().__init__(name=f"Move le bras Gauche {self.pos_info}")
        self.bb, self.robot, self.world = get_bb_robot(self)
        self.pos = pos

    def initialise(self) -> None:
        print(f"Move le bras Gauche {self.pos}")
        
    def update(self):
        if self.robot.actionneurs.moveG(self.pos):
            return py_trees.common.Status.SUCCESS
        else :
            return py_trees.common.Status.FAILURE
...
class Relacher(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        if self.robot.relacher(self.cote,self.color_caisse):
            return py_trees.common.Status.SUCCESS
        else :
            return py_trees.common.Status.FAILURE
